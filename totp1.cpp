    ...
    void decode_base32_secret(const std::string &base32_secret) {
        ...
        for (size_t i = 0, index = 0; i < base32_secret.size(); ) {
            ...
            while (bits_remaining) {
                ...
                uint8_t bits_to_write = std::min<unsigned int>(5, bits_remaining);
...
public:
    ...
    char temp_totp[7]; // Change this line to create an array of size 7 (6 digits + null-terminator)
    oath_hotp_generate(reinterpret_cast<const char *>(decoded_secret), decoded_secret_len, counter, 6, 0, 0, temp_totp);
    temp_totp[6] = '\0'; // Make sure to null-terminate the string

    unsigned int totp = static_cast<unsigned int>(std::stoi(temp_totp));
